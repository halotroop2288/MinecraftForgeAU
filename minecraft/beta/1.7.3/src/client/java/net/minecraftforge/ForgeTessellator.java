 ...
 * @author Eloraam
 */
public class ForgeTessellator extends Tessellator {
	protected static final int nativeBufferSize = 0x200000;
	protected static final int trivertsInBuffer = (nativeBufferSize / 48) * 6;

	protected final static boolean     sharedUseVBO      = getCapabilities().GL_ARB_vertex_buffer_object;
	protected final static ByteBuffer  sharedByteBuffer  = GLAllocation.createDirectByteBuffer(nativeBufferSize * 4);
	...
	static {
		if (sharedUseVBO) {
			sharedVertexBuffers = GLAllocation.createDirectIntBuffer(sharedVboCount);
	...
	public ForgeTessellator(int bufferSize) {
		super(bufferSize);
		this.rawBuffer = null;
	...
	@Override
	public void draw() {
		if (!this.isDrawing) throw new IllegalStateException("Not tessellating!");

		this.isDrawing = false;
		int offs = 0;

		while (offs < vertexCount) {
			int vtc = min(vertexCount - offs, this.drawMode == 7 && convertQuadsToTriangles
				? trivertsInBuffer : nativeBufferSize >> 5);

			sharedIntBuffer.clear();
			sharedIntBuffer.put(rawBuffer, offs * 8, vtc * 8);
			...
			if (this.drawMode == 7 && convertQuadsToTriangles) glDrawArrays(4, 0, this.vertexCount);
			...
			if (drawMode == GL_QUADS && convertQuadsToTriangles) glDrawArrays(GL_TRIANGLES, 0, vtc);
