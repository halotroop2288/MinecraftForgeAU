...
public class Configuration {
	...
	public @Nullable Property getOrCreateBlockIdProperty(@NotNull String key, int defaultID) {
		if (configBlocks == null) {
			configBlocks = new boolean[Block.BY_ID.length];
		...
		if (Block.BY_ID[defaultID] == null && !configBlocks[defaultID]) {
			property.value = Integer.toString(defaultID);
			configBlocks[defaultID] = true;
			return property;
		}

		for (int i = Block.BY_ID.length - 1; i >= 0; i--) {
			if (Block.BY_ID[i] == null && !configBlocks[i]) {
