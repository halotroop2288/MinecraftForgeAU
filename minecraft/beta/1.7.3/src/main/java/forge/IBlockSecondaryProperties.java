...
public interface IBlockSecondaryProperties {
	...
	default boolean isBlockNormalCube(@NotNull World world, int x, int y, int z) {
		if (this instanceof Block) {
			...
			return block.material.isOpaque() && block.isCube();
		}
	 ...
	 * Provides an override to the corresponding section in {@link World#canPlace(int, int, int, int, boolean, int)}.
	 *
	 * @param world the world to act on
	 * @param x     the x position of the block
	 * @param y     the y position of the block
	...
	default boolean isBlockReplaceable(@NotNull World world, int x, int y, int z) {
		if (this instanceof Block) {
			Block block = ((Block) this);
			return block == Block.FLOWING_WATER || block == Block.WATER
				|| block == Block.FLOWING_LAVA || block == Block.LAVA
	...
	/**
	 * Provides an override to the related test within {@link World#containsFireSource(Box)}.
	 *
	 * @param world the world to act on
	 * @param x     the x position of the block
	 * @param y     the y position of the block
	...
	default boolean isBlockBurning(@NotNull World world, int x, int y, int z) {
		if (this instanceof Block) {
			Block block = ((Block) this);
			return block == Block.FIRE || block == Block.FLOWING_LAVA || block == Block.LAVA;
		}
	 ...
	 * Provides an override to {@link World#isAir(int, int, int)}
	 *
	 * @param world the world to act on
	 * @param x     the x position of the block
	 * @param y     the y position of the block
	...
	default boolean isAirBlock(@NotNull World world, int x, int y, int z) {
		return this instanceof Block && ((Block) this).id == 0;
	}
}
